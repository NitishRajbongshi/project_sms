...
if($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    $sql = "SELECT * FROM `student_login` WHERE `student_phone` = '$phone'";
    $result = mysqli_query($conn, $sql);
    $no_of_rows = mysqli_num_rows($result);
    if($no_of_rows > 0) {
    ...
    if(($rollno_duplicacy == false) && ($email_duplicacy == false) && ($phone_duplicacy == false)) {
        ...
        $sql = "INSERT INTO `student_login`(`rollno`, `student_email`, `student_phone`, `student_name`, `student_password`, `academic_year`) VALUES ('$rollno','$email','$phone','$name','$hash_password','$academic_year')" ;
        $result = mysqli_query($conn, $sql);
        if($result) {
            echo ' 
            <script>
